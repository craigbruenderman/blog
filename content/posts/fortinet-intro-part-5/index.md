...
In this part, I'll dive into the SD-WAN feature of Fortinet Fortigates.
...
* Hub devices utilize 2 loopbacks:
    * Hub#-Lo: Used for health checks from branch devices to measure VPN tunnel performance.
    * BGP-Lo: Used for BGP peering between branch and hub devices.
* Branch devices utilize one loopback:
    * Branch#-Lo: Used to form BGP peering with hubs and other branches when ADVPN shortcuts are established.
* Autonomous System number for BGP:
    * A private number is used and must remain exclusively for this SD-WAN BGP configuration.
    * A second BGP AS is used for HUB to DC peering.

## SD-WAN Deployment

* The SD-WAN overlay orchestrator generates the necessary IPsec, BGP and CLI provisioning templates that are required for the creation of your SD-WAN overlays